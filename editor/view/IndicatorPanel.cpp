...
LRESULT IndicatorPanel::OnNCPaint(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam){
	...
	if (vscroll && hscroll){
		...
		res = FillRgn(hdc, combinedRG, hbrPanelBg);

		DeleteObject(combinedRG);
		DeleteObject(underRG);

	}else{
		res = FillRgn(hdc, prRG, hbrPanelBg);
	}

	DeleteObject(hbrPanelBg); // CreateSolidBrush must be released, unlike GetSysColorBrush
	...
	for (auto it = m_Indicators.begin(); it != m_Indicators.end(); it++)
	{
		...
		if (mask)
		{
			...
			if (brush != NULL)
			{
				...
				long tx = m_PanelRect.left; // no padding - flush with panel edge
				t = { tx, curOffset, tx + panelWidth, curOffset + g_indicator_height };
...
void IndicatorPanel::paintIndicators(HDC hdc){
	...
	RECT t = m_PanelRect; // no left offset - clear the whole (now narrow) panel

	HBRUSH hbrPanelBg = CreateSolidBrush(g_color_panel_bg); // #171717 instead of system 3D-face color
	FillRect(hdc, &t, hbrPanelBg);
	DeleteObject(hbrPanelBg);
